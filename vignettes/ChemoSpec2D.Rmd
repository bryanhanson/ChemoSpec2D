...
acknowledgements: I'd like to thank Teddy Zartler of Pfizer for encouragement in developing this package, and for providing valuable test data sets.
...
* PARAFAC. PARAFAC, or parallel factor analysis, is very similar to TUCKER3 except that each dimension is reduced to the *same* number of components. `ChemoSpec2D` can carry out PARAFAC and it is discussed in greater detail below.
* $N$-way Image Analysis.  In the case of 2D NMR spectra $N$ would be three.  This is a general term term which includes processes such as the TUCKER variations and PARAFAC.
